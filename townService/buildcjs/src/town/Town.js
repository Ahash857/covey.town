...
const GameAreaFactory_1 = __importDefault(require("./games/GameAreaFactory"));
...
class Town {
    ...
    async addPlayer(userName, socket) {
        const newPlayer = new Player_1.default(userName, socket.to(this._townID));
        ...
        socket.on('interactableUpdate', (update) => {
            if ((0, TestUtils_1.isViewingArea)(update)) {
        ...
        socket.on('interactableCommand', (command) => {
            ...
            if (interactable) {
                try {
                    ...
                    if (err instanceof InvalidParametersError_1.default) {
                        socket.emit('commandResponse', {
                            commandID: command.commandID,
                            interactableID: command.interactableID,
                            isOK: false,
    ...
    _updatePlayerLocation(player, location) {
        ...
        if (!(prevInteractable === null || prevInteractable === void 0 ? void 0 : prevInteractable.contains(location))) {
            if (prevInteractable) {
            ...
            location.interactableID = newInteractable === null || newInteractable === void 0 ? void 0 : newInteractable.id;
        }
        else {
    ...
    initializeFromMap(map) {
        ...
        const viewingAreas = objectLayer.objects
            ...
            .map(eachViewingAreaObject => ViewingArea_1.default.fromMapObject(eachViewingAreaObject, this._broadcastEmitter));
        const conversationAreas = objectLayer.objects
            .filter(eachObject => eachObject.type === 'ConversationArea')
            .map(eachConvAreaObj => ConversationArea_1.default.fromMapObject(eachConvAreaObj, this._broadcastEmitter));
        const gameAreas = objectLayer.objects
            .filter(eachObject => eachObject.type === 'GameArea')
            .map(eachGameAreaObj => (0, GameAreaFactory_1.default)(eachGameAreaObj, this._broadcastEmitter));
